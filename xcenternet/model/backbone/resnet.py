...
def create_resnet(height, width, pretrained: bool, mode: XModelMode = XModelMode.SIMPLE):
    ...
    base_model, features = upsample(
        base_model, x, ["conv2_block3_out", "conv3_block3_out", "conv4_block3_out", "conv5_block3_out"], mode
    )

    return base_model, inputs, features


def create_resnet_18(height, width, pretrained: bool, mode: XModelMode = XModelMode.SIMPLE):
    shape = (height, width, 3)
    ...
    inputs = tf.keras.Input(shape=shape, name="input")
    base_model, features = upsample(
        base_model, inputs, ["stage2_unit1_relu1", "stage3_unit1_relu1", "stage4_unit1_relu1", "relu1"], mode
    )
