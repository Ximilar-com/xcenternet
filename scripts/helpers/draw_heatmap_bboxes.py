...
parser.add_argument("--bboxes", type=list, default=[[[ 304, 582, 422, 705 ], [20, 0, 25, 1], [ 304, 582, 422, 705 ], [ 238, 290, 465, 988 ], [ 38, 290, 665, 988 ], [ 38, 490, 665, 788 ], [1000,1000, 1024, 1024]],  [[ 304, 582, 422, 705 ]]], help="bounding boxes")
...
heatmap_shape = (2, args.image_size, args.image_size, len(args.labels[0]))
labels = [np.asarray([0 for i in range(len(args.bboxes[k]))]) for k in range(len(args.bboxes))]
print(labels)
if args.ttf_version:
    heatmap, box_target, reg_weight, _, seg_map, seg_mask = draw_heatmaps_ttf(
        heatmap_shape, [np.asarray(boxes) for boxes in args.bboxes], labels,
    ...
    print("SEG MASK SHAPE:", seg_mask.shape, seg_mask[0].shape)
    seg_map = seg_map[0]
    seg_mask = seg_mask[0]
    test = np.reshape(seg_mask[:,:,207], (1024,1024, 1)).astype(np.uint8) * 255
    test = tf.constant(test)
    test = tf.image.encode_png(tf.image.grayscale_to_rgb(test))
    tf.io.write_file("heatmap_S.jpg", test)
else:
